...
def verify_admin_access(request: Request) -> bool:
    ...
    if len(ADMIN_API_KEY) < MIN_API_KEY_LENGTH:
        ...
        logger.warning(f"ADMIN_API_KEY is configured with insecure length: {len(ADMIN_API_KEY)} (min: {MIN_API_KEY_LENGTH})")
    ...
    if auth_header.startswith("Bearer "):
        ...
        if token and token == ADMIN_API_KEY and len(token) >= MIN_API_KEY_LENGTH:
            ...
            logger.debug(f"Admin access granted via Bearer token from {request.client.host if request.client else 'unknown'}")
            return True
