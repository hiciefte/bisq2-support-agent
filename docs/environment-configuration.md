...
* **`REACTOR_IDENTITY_SALT`**
    * Description: Stable secret salt used to hash reaction identities for privacy-safe ownership checks and deduplication.
    * Required: Yes in production
    * Production source: generated by `scripts/deploy.sh` and stored under `$BISQ_SUPPORT_SECRETS_DIR/reactor_identity_salt`.
*   **`EXPOSE_API_PORT`**
...
* **`GRAFANA_DATASOURCE_API_KEY`**
    * Description: Dedicated read-only API key used only by Grafana's Infinity datasource for dashboard analytics.
    * Required: Yes in production
    * Production source: generated by `scripts/deploy.sh` and stored under `$BISQ_SUPPORT_SECRETS_DIR/grafana_datasource_api_key`.
